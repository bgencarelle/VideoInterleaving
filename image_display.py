...
def run_display(clock_source=CLOCK_MODE):
    ...
    # Helper callback to update the triple buffer when asynchronous loading completes.
    def async_load_callback(fut, scheduled_index):
        try:
            result = fut.result()
            triple_buffer.update(scheduled_index, result)
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        while state.run_mode:
            ...
            if new_index != previous_index:
                ...
                # Retrieve the latest image pair from the triple buffer using our tolerance window.
                ...
                if result is not None:
                    ...
                    # Continue displaying the current textures if no buffered image is available.
                ...
                if index == 0:
                    ...
                    # Infer the direction from the change: if current index > previous, we're ascending.
