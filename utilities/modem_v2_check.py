...
def receive_for(args,layout,coder):
    # WAV checks exercise the same recovered-clock path as live reception.
    # Explicit Receiver callers can still request full-packet EQ recovery.
    return V2.Receiver(layout,coder,recovery=False,fast=True)
...
def do_live_receive(args):
    ...
    from animation_modem.waveform_receiver import Receiver as WaveformReceiver
    receiver_factory = {'progressive': ProgressiveReceiver, 'reference': ReferenceReceiver,
...
def main(argv=None):
    ...
    lr.add_argument('--receiver', choices=('progressive', 'reference', 'waveform'), default='progressive')
