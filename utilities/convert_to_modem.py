...
def bake_tree(source, output, profile='color', jpeg_layout='sbs'):
    ...
    valid_profiles = set(PROFILES.keys()).union({'color-dct'})
    if profile not in valid_profiles or jpeg_layout not in ('sbs', 'rgb'):
        raise ValueError(f'Unsupported profile ({profile}) or JPEG layout ({jpeg_layout})')
    ...
    # Assign target resolution (80x96 for color-dct, otherwise read from PROFILES)
    size = (80, 96) if profile == 'color-dct' else PROFILES[profile][0]
...
def main():
    profile_choices = sorted(list(set(PROFILES.keys()).union({'color-dct'})))
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument('-i', '--input-dir', required=True, type=Path)
    p.add_argument('-o', '--output-dir', type=Path)
    p.add_argument('--profile', choices=profile_choices, default='color')
    ...
    try:
        bake_tree(a.input_dir, out, a.profile, a.jpeg_layout)
