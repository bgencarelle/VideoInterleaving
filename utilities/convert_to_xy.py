...
# Worker memmaps are deliberately hidden and renamed only after a folder is
# complete. These exact prefixes are therefore safe to identify as baker
# scratch files; ordinary user files and completed arrays never match them.
TEMP_PREFIXES = (".thumbs-", ".stipple-xy-", ".stipple-lae-",
                 ".stipple-mass-")
...
def _baker_temp_files(root):
    root = Path(root)
    if not root.is_dir():
        return []
    return sorted(
        p for p in root.rglob("*.npy")
        if p.name.startswith(TEMP_PREFIXES) and p.is_file()
    )


def clean_baker_temp_files(root):
    """Remove only unfinished memmaps owned by this baker.

    Cleanup is explicit at startup so a second concurrently launched baker
    cannot silently delete the first one's live files.
    """
    paths = _baker_temp_files(root)
...
def estimate_fixed_storage(tasks):
    """Exact payload estimate for thumbnails and stipple stores.

    One standardized source frame is read per folder. Geometry is intentionally
    excluded: its compact variable-size arrays are small beside these fixed
    stores and cannot explain a multi-gigabyte surprise. ``largest`` is kept
    for a useful diagnostic if the aspect ratio of one folder is pathological.
    """
    total = thumb_total = stipple_total = frames = 0
    largest = None
    for src_folder, _dest, prof in tasks:
        files = _image_files(src_folder)
        if not files:
            continue
        _rgb, alpha = load_rgba(files[0])
        h, w = alpha.shape
        tw = int(prof.get("thumb_width", THUMB_W))
        th = max(1, int(round(h * tw / float(w))))
        count = len(files)
        candidates = int(prof.get(
            "stipple_candidates", STIPPLE_CANDIDATES))

        # thumbnail: uint8 [luminance, alpha]
        thumb_bytes = count * th * tw * 2
        # candidates: uint16 xy + uint8 luminance/alpha/edge + float32 mass
        stipple_bytes = count * (candidates * (2 * 2 + 3) + 4)
        fixed_bytes = thumb_bytes + stipple_bytes
        detail = {
            "folder": Path(src_folder), "frames": count,
            "source_shape": (h, w), "thumb_shape": (th, tw),
            "bytes": fixed_bytes,
        }
        if largest is None or fixed_bytes > largest["bytes"]:
            largest = detail
        total += fixed_bytes
        thumb_total += thumb_bytes
        stipple_total += stipple_bytes
        frames += count
    return {
        "bytes": total, "thumb_bytes": thumb_total,
        "stipple_bytes": stipple_total, "frames": frames,
        "folders": len(tasks), "largest": largest,
    }
...
def process_folder(args):
    ...
    try:
        ...
        # KeyboardInterrupt and other BaseException paths bypass the ordinary
        # worker error return. A Python-level interruption still reaches this
        # block, preventing the one-huge-hidden-file-per-folder failure mode.
        thumbs = None
        stipple_xy = stipple_lae = stipple_mass = None
        for temp in (thumb_temp, stipple_xy_temp, stipple_lae_temp,
                     stipple_mass_temp):
...
def main():
    ...
    ap.add_argument("--max-bake-gb", type=float, default=MAX_BAKE_GB,
                    metavar="GB",
                    help=f"refuse a bake whose fixed thumbnail/stipple stores "
                         f"are estimated above this many decimal GB (default "
                         f"{MAX_BAKE_GB:g}; 0 disables the ceiling)")
    ap.add_argument("--clean-stale-temp", action="store_true",
                    help="remove unfinished .thumbs-* and .stipple-* worker "
                         "memmaps from the destination before the size check")
    ...
    if not math.isfinite(args.max_bake_gb) or args.max_bake_gb < 0:
        ap.error("--max-bake-gb must be finite and non-negative")
    ...
    stale = _baker_temp_files(output_root)
    if stale:
        stale_size = sum(p.stat().st_size for p in stale)
        if not args.clean_stale_temp:
            logging.error(
                "Found %d unfinished baker temp file(s), occupying %s under "
                "%s. A previous bake was interrupted. Re-run with "
                "--clean-stale-temp after confirming no other baker is active.",
                len(stale), _human_bytes(stale_size), output_root)
            return 1
        removed, removed_size = clean_baker_temp_files(output_root)
        logging.warning("Removed %d stale baker temp file(s), reclaiming %s.",
                        removed, _human_bytes(removed_size))

    try:
        estimate = estimate_fixed_storage(tasks)
    except Exception as e:
        logging.error("Bake preflight could not read a representative frame: %s",
                      e)
        return 1

    free = shutil.disk_usage(_existing_ancestor(output_root)).free
    logging.info(
        "Preflight: %d folders / %d frames; fixed stores %s "
        "(thumbnails %s + stipple %s); free %s",
        estimate["folders"], estimate["frames"],
        _human_bytes(estimate["bytes"]),
        _human_bytes(estimate["thumb_bytes"]),
        _human_bytes(estimate["stipple_bytes"]), _human_bytes(free))
    largest = estimate["largest"]
    if largest is not None:
        logging.info(
            "Largest folder: %s -- %d frames, source %dx%d -> thumbnail %dx%d, "
            "fixed stores %s",
            largest["folder"], largest["frames"],
            largest["source_shape"][1], largest["source_shape"][0],
            largest["thumb_shape"][1], largest["thumb_shape"][0],
            _human_bytes(largest["bytes"]))

    limit = args.max_bake_gb * 1_000_000_000
    if limit and estimate["bytes"] > limit:
        logging.error(
            "Refusing bake: estimated fixed stores are %s, above the %g GB "
            "safety ceiling. Check source aspect ratios and --thumb-width. "
            "If intentional, raise --max-bake-gb explicitly.",
            _human_bytes(estimate["bytes"]), args.max_bake_gb)
        return 1
    required_free = int(estimate["bytes"] * 1.10)
    if required_free > free:
        logging.error(
            "Refusing bake: %s free, but fixed stores plus 10%% working "
            "headroom require %s.", _human_bytes(free),
            _human_bytes(required_free))
        return 1
