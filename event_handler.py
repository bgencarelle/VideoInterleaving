        ...
        if state.fullscreen:
            ...
            try:
                ...
                if is_macos:
                    ...
                    # is not a valid fullscreen-loss signal. Ignore the
                    # resize callback generated by our own resize when the
                    # window already matches the monitor mode; otherwise the
                    # display reinitializes forever.
                    monitor = glfw.get_primary_monitor()
                    mode = glfw.get_video_mode(monitor) if monitor else None
                    if mode is not None and (
                        width != mode.size.width or height != mode.size.height
                    ):
