...
# Use preserved transport code; keep the archived entry point unchanged.
sys.path.insert(0, str(Path(__file__).resolve().parent / 'source'))

from animation_modem.audio_common import device, pair, sounddevice, wav_blocks
from animation_modem.capture import AudioGate, BufferedInput, CaptureHealth, CaptureResampler, open_input
from animation_modem.imaging import DEFAULT_PROFILE, fit_shapes, plane_shapes, values_image
from animation_modem.transport2 import PRESETS, RATE, SourceCoder
from animation_modem.progressive import Receiver as ProgressiveReceiver
from animation_modem.transport2 import Receiver as PacketReceiver

def Receiver(layout, coder):
    """Match utilities/modem_v2_check.py receive_for exactly."""
    return PacketReceiver(layout, coder, recovery=False, fast=True)

from animation_modem.reference_receiver import Receiver as ReferenceReceiver
from animation_modem.timing import expand_timestamp, ProgressSummary, TimingStats
from animation_modem.presentation import DeadlinePresentationBuffer
from animation_modem.impairments import Emulator, add_arguments, settings_from_args
...
def live_results(sd, input_device, channels, layout, coder, settings, stop, report,
    ...
    # Use the selected receiver without changing capture or shared-time scheduling.
...
def main(argv=None):
    ...
    p.add_argument('--receiver', choices=('packet', 'progressive', 'reference'), default='packet',
                   help='packet: original modem 2 (default); progressive/reference: optional experimental receivers')
    ...
    def receive():
        receiver_factory = {'packet': Receiver, 'progressive': ProgressiveReceiver,
                            'reference': ReferenceReceiver}[args.receiver]
        receiver = receiver_factory(layout, coder)
        emulator = Emulator(settings)
        def process(audio):
